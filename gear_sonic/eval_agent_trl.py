...
def main(override_config: omegaconf.OmegaConf):
    ...
    if config.get("export_onnx_only", False):
        ...
        if "tokenizer" in example_obs_dict and has_encoders:
            ...
            # Fused (single-encoder + decoder) exports, one per encoder present in the model.
            # Configs without an SMPL encoder (e.g. sonic_ffmaster) must not export _smpl.onnx.
            present_encoders = list(model.policy.actor_module.encoders_to_iterate)
            for encoder_name in ("smpl", "g1", "teleop"):
                if encoder_name not in present_encoders:
                    continue
                inference_helpers.export_universal_token_module_as_onnx(
                    ...
                    exported_model_name=exported_onnx_name.replace(".onnx", f"_{encoder_name}.onnx"),
                    batch_size=1,
                )
